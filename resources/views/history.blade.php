            ...
            <div class="space-y-6">
                @forelse ($applicants as $applicant)
                    <div class="bg-white overflow-hidden shadow-sm rounded-2xl p-6 border border-gray-200">
                        {{-- Header kartu --}}
                        <div class="flex flex-col sm:flex-row justify-between sm:items-center mb-4 pb-4 border-b border-gray-100">
                            <div>
                        ...
                        <div>
                            ...
                            <div class="flex items-center justify-between">
                                @php
                                    ...
                                    $currentStatusInfo = $stageMapping[$applicant->status] ?? ['index' => -1, 'result' => 'unknown'];
                                ...
                                @foreach ($stages as $index => $stageName)
                                    @php
                                        $isPassed = $index < $currentStageIndex || ($index == $currentStageIndex && $currentResult == 'passed');
                                    ...
                                    <div class="flex flex-col items-center flex-1 min-w-0">
                                        ...
                                        <p class="text-center text-xs mt-2 w-24 whitespace-normal
                                    ...
                                    @if (!$loop->last)
                                        <div class="flex-auto border-t-2 {{ $isActive ? 'border-green-500' : 'border-gray-200' }}"></div>
                                    @endif
                                @endforeach
                            </div>
                        </div>
                        ...
                        @if ($applicant->status === 'Tes Tulis')
                            @php
                                // Ambil relasi test apa adanya (tanpa optional())
                                $t = $applicant->position?->test;
                            @endphp

                            @if ($applicant->position && $t && filled($t->slug))
                                @php
                                    $now   = now();
                                    $open  = $t->test_date;
                                    $close = $t->test_closed;
                                    ...
                                    // Buat signed URL hanya jika slug ada
                            ...
                            @else
                                {{-- Fallback: Tes Tulis belum tersedia / slug kosong --}}
                                <div class="mt-6 p-4 rounded-xl border border-indigo-200 bg-indigo-50">
                                    <h4 class="text-md font-semibold text-indigo-800 mb-2">Tes Tulis</h4>
                                    <div class="text-sm text-gray-600">
                                        Jadwal atau data Tes Tulis belum ditentukan untuk posisi ini. Silakan cek kembali secara berkala.
                                    </div>
                                </div>
                            @endif
                        @endif
