...
<x-app-admin>
  {{-- (Sesuai permintaan) tidak menampilkan alert success standar --}}
  ...
  <div x-data="scoreModal()" x-init="init()" x-cloak>
    ...
    <form method="GET" class="bg-white border rounded-lg p-4 mb-5"
          action="{{ route('admin.applicant.seleksi.technical_test') }}">
      <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
        <div>
          <label class="text-sm text-gray-600">Batch</label>
          <select name="batch" class="w-full border rounded px-3 py-2">
            <option value="">— Semua —</option>
            @foreach ($batches as $b)
              <option value="{{ $b->id }}"
                @selected((string)($selectedBatchId ?? request('batch', request('batch_id'))) === (string)$b->id)>
                {{ $b->name ?? ('Batch #'.$b->id) }}
        ...
        <div>
          ...
          <select name="position_id" class="w-full border rounded px-3 py-2">
            ...
            @foreach ($positions as $p)
              <option value="{{ $p->id }}"
                @selected((string)($selectedPosId ?? request('position_id', request('position'))) === (string)$p->id)>
                {{ $p->name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="md:col-span-2">
          ...
          <input type="text" name="q" value="{{ $q ?? request('q', request('search')) }}"
                 class="w-full border rounded px-3 py-2" placeholder="Ketik nama atau email...">
        </div>
      </div>
      <div class="mt-3 flex items-center gap-2">
        <button class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Terapkan</button>

        {{-- Reset: tetap pertahankan batch (opsional), bersihkan filter lainnya --}}
        <a href="{{ route('admin.applicant.seleksi.technical_test', array_filter([
              'batch' => $selectedBatchId ?? request('batch')
        ])) }}" class="px-4 py-2 border rounded hover:bg-gray-50">Reset</a>
      </div>
    </form>
              ...
              <tr class="border-t">
                ...
                <td class="px-4 py-2">
                  <button type="button"
                          ...
                          @click="openWith({
                            action: '{{ route('admin.applicant.seleksi.technical_test.answers.update', $a) }}',
                            name: @js($app->name ?? '—'),
                            email: @js($app->email ?? ''),
                            posisi: @js($pos->name ?? '—'),
