...
<x-app-admin>
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg py-4">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-4 py-3 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                {{-- EDIT: panggil modal edit terpisah --}}
                                                <button
                                                    type="button"
                                                    ...
                                                    onclick="openEditModal({{ json_encode([
                                                        ...
                                                        'deadline' => $position->deadline ? $position->deadline->format('Y-m-d') : null,
                                                        'description' => is_array($position->description) ? $position->description : (json_decode($position->description, true) ?? []),
                                                        'skills' => is_array($position->skills) ? $position->skills : (json_decode($position->skills, true) ?? []),
                                                        'requirements' => is_array($position->requirements) ? $position->requirements : (json_decode($position->requirements, true) ?? []),
                                                        'majors' => is_array($position->majors) ? $position->majors : (json_decode($position->majors, true) ?? []),
                                                ...
                                                {{-- DELETE --}}
                                                <form action="{{ route('position.destroy', $position->id) }}" method="post" class="inline" onsubmit="return confirm('Anda yakin?')">
                                                    @method('delete') @csrf
        ...
        {{-- include modal add position (existing partial) --}}
        @include('admin.batch.partials.modal-add-position', ['batch' => $batch])

        {{-- include modal edit position (terpisah) --}}
        @include('admin.batch.partials.modal-edit-position') {{-- pastikan file path sesuai --}}
    </div>

    {{-- Script: fungsi membuka modal edit dan mengisi form --}}
    <script>
        function openEditModal(data, updateUrl) {
            // data: object dengan keys: id,name,quota,status,pendidikan_minimum,deadline,description[],skills[],requirements[],majors[]
            // isi form di modal edit (bootstrap)
            ...
            // convert arrays to newline strings
